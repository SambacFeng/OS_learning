...
int main() {
	...
	if(fd1 == -1)
		...
		printf("Fifo1 opened for reading!\n");
	...
	if(fd2 == -1)
		...
		printf("Fifo2 opened for writing!\n");
...
void *readMsg(void *param) {
	...
	while(flag) 
	{
		read(pp->fd1, pp->buf1, SIZE);
		printf("\r[B]: %s", pp->buf1);
...
void *writeMsg(void *param) {
	...
	{
		printf("[A]: ");
		fgets(pp->buf2, SIZE, stdin);
		write(pp->fd2, pp->buf2, SIZE);
